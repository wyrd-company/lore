...
func New(pool *pgxpool.Pool, ingestToken, adminToken string) http.Handler {
	server := &Server{
		pool: pool, sync: synchronization.NewRepository(pool), ingestToken: ingestToken, adminToken: adminToken,
	}
	...
	mux.Handle("GET /api/projects/{project}/search", projectScope(pool, http.HandlerFunc(server.stubSearch)))
...
func (s *Server) stubSearch(w http.ResponseWriter, r *http.Request) {
	writeStub(w, r, "search")
}
