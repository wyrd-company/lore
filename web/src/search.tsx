...
export function SearchPage() {
  ...
  const active = [...selectedTypes.map((value) => ["sourceType", value]), ...params.getAll("repository").map((value) => ["repository", value]), ...params.getAll("branch").map((value) => ["branch", value]), ...params.getAll("tag").map((value) => ["tag", value])] as string[][];
  ...
  return <div className="l-page"><div className="lore-page-head"><span className="page-kicker">Hybrid retrieval</span><h1 className="lore-page-head__title">Search the archive</h1></div>
    ...
    <div className="search-layout"><aside aria-label="Search filters">
    ...
    </aside><section aria-live="polite">
      {active.length > 0 && <div className="active-filters">{active.map(([key, value]) => <button className="lore-chip lore-chip--removable" key={`${key}-${value}`} onClick={() => toggle(key, value)}>{value} <span className="lore-chip__x">×</span></button>)}</div>}
...
function FilterGroup({ title, children }: { title: string; children: React.ReactNode }) { return <fieldset className="facet-group"><legend>{title}</legend><div className="facet-stack">{children}</div></fieldset>; }
