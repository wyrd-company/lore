...
function AnnotationCard({ project, annotation, active, revisions, attribution, onActive, mutate }: { project: string; annotation: Annotation; active: boolean; revisions: RevisionSummary[]; attribution: string; onActive: () => void; mutate: (operation: () => Promise<unknown>) => Promise<void> }) {
  const [targetRevision, setTargetRevision] = useState(""); const [reply, setReply] = useState(""); const [replying, setReplying] = useState(false); const eligible = revisions.filter((item) => item.id !== annotation.revisionIdentity);
  ...
  const submitReply = () => mutate(async () => { await api.replyToAnnotation(project, annotation.id, { body: reply, attributedUsername: attribution }); setReply(""); setReplying(false); });
  return <article className={`lore-anno ${active ? "is-active" : ""}`} data-state={annotation.status} tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") onActive(); }} onClick={onActive}><div className="lore-anno__top"><span className="lore-status" data-state={annotation.status}>{annotation.status}</span><span className="lore-anno__author">{annotation.attributedUsername}</span><time className="lore-anno__time" dateTime={annotation.updatedAt}>{relativeTime(annotation.updatedAt)}</time></div>{annotation.selectedQuote && <blockquote className="lore-anno__quote">{annotation.selectedQuote}</blockquote>}<p className="lore-anno__body">{annotation.body}</p><ReplyThread annotation={annotation} />{annotation.copiedFromAnnotationId && <div className="lore-anno__prov">↪ copied from {annotation.copiedFromAnnotationId.slice(0, 8)}</div>}{annotation.priorTarget && <div className="lore-anno__prov">↪ moved from an earlier revision</div>}<div className="lore-anno__actions" onClick={(event) => event.stopPropagation()}><button className="lore-btn lore-btn--ghost lore-btn--sm" onClick={() => setReplying((value) => !value)}>Reply</button>{annotation.status === "open" ? <><button className="lore-btn lore-btn--ghost lore-btn--sm" onClick={() => update("resolved")}>Resolve</button><button className="lore-btn lore-btn--ghost lore-btn--sm" onClick={() => update("dismissed")}>Dismiss</button></> : <button className="lore-btn lore-btn--ghost lore-btn--sm" onClick={() => update("open")}>Reopen</button>}</div>{replying && <div className="annotation-reply-composer" onClick={(event) => event.stopPropagation()}><textarea className="lore-textarea" aria-label="Reply" value={reply} onChange={(event) => setReply(event.target.value)} autoFocus /><div><button className="lore-btn lore-btn--ghost lore-btn--sm" onClick={() => setReplying(false)}>Cancel</button><button className="lore-btn lore-btn--primary lore-btn--sm" disabled={!reply.trim()} onClick={submitReply}>Reply</button></div></div>}{eligible.length > 0 && <div className="retarget-controls" onClick={(event) => event.stopPropagation()}><select className="lore-select" aria-label="Target revision" value={targetRevision} onChange={(event) => setTargetRevision(event.target.value)}><option value="">Revision…</option>{eligible.map((item) => <option value={item.id} key={item.id}>{item.current ? "Current" : shortHash(item.contentHash)}</option>)}</select><button className="lore-btn lore-btn--ghost lore-btn--sm" disabled={!targetRevision} onClick={() => retarget("copy")}>Copy</button><button className="lore-btn lore-btn--ghost lore-btn--sm" disabled={!targetRevision} onClick={() => retarget("move")}>Move</button></div>}</article>;
}

function ReplyThread({ annotation }: { annotation: Annotation }) {
  if (!annotation.replies.length) return null;
  return <div className="annotation-replies" aria-label={`${annotation.replies.length} replies`}>{annotation.replies.map((reply) => <div className="annotation-reply" key={reply.id}><div><strong>{reply.attributedUsername}</strong><time dateTime={reply.createdAt}>{relativeTime(reply.createdAt)}</time></div><p>{reply.body}</p></div>)}</div>;
}
